...
flexdashboard creates a dashboard framework for you to plug in your data, visuals, descriptions, and materials. 
...
This creates a special kind of **RMarkdown** file. You write R code to handle the data, plain text with markup to style it, and use the functions in flexdashbaord to set the layout you want - `r ct(corange, "all in the same file")`. 
...
RStudio gives you a basic template by default. We'll go through the pieces of it next.
...
**YAML** sets the title and output format. The default values are fine so you don't have to do anything here unless you want something specific. Spacing and indentation matter in YAML (annoying but usually handled for you).
...
```Column``` followed by ```---------``` creates a new column.  

The enture screen is 1,000 units wide, and ```{data-width = x}``` controls the widths of the columns. 
...
```###``` creates a flexbox in the column (titled "Where we are"). The flexbox is where your graph, text, etc. will be displayed. Think of flexboxes like smart containers for your content.
...
``` '''{r}``` defines a chunk of R code. This evaluates when you compile and whatever it produces gets inserted into the flexbox associated with that chunk.
